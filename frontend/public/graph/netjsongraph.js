...
(function () {
  ...
  d3._getPosition = function (element, container) {
    var n = element.node()
    var nPos = n.getBoundingClientRect()
  ...
  d3.netJsonGraph = function (url, opts) {
    ...
    opts = d3._extend({
      ...
      prepareData: function (graph) {
        var nodesMap = {}
        var nodes = graph.nodes.slice() // copy
        var links = graph.links.slice() // copy
        var nodes_length = graph.nodes.length
        var links_length = graph.links.length
        ...
        for (var c = 0; c < links_length; c++) {
          ...
          var targetIndex = nodesMap[links[c].target]
      ...
      onClickNode: function (n) {
        ...
        var overlayInner = d3.select('.njg-overlay > .njg-inner')
    ...
    var width = d3._pxToNumber(el.style('width'))
    var height = d3._pxToNumber(el.style('height'))
    ...
    // width is easy to get, if height is 0 take the height of the body
      .size([width, height])
    var zoom = d3.behavior.zoom().scaleExtent(opts.scaleExtent)
    // panner is the element that allows zooming and panning
    var panner = el.append('svg')
      .attr('width', width)
      .attr('height', height)
      .call(zoom.on('zoom', opts.redraw))
      .append('g')
      .style('position', 'absolute')
    var svg = d3.select(opts.el + ' svg')
    var drag = force.drag()
    var overlay = d3.select(opts.el).append('div').attr('class', 'njg-overlay')
    var closeOverlay = overlay.append('a').attr('class', 'njg-close')
    var overlayInner = overlay.append('div').attr('class', 'njg-inner')
    var metadata = d3.select(opts.el).append('div').attr('class', 'njg-metadata')
    var metadataInner = metadata.append('div').attr('class', 'njg-inner')
    var closeMetadata = metadata.append('a').attr('class', 'njg-close')
    ...
    processJson = function (graph) {
      ...
      var groups = panner.selectAll('.node')
        .data(nodes)
        .enter()
        .append('g')
    ...
    if (typeof (url) === 'object') {
      ...
      d3.json(url, function (error, graph) {
        ...
        if (graph.type === 'NetworkCollection') {
          ...
          var select = selectGroup.append('select')
            .attr('id', 'select')
          str = graph
